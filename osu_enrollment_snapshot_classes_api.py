...
def fetch_class_search(srcdb: str, keyword: str) -> Dict[str, Any]:
    ...
    for attempt in range(1, max_tries + 1):
        ...
        try:
            ...
            raise RuntimeError(
                f"Class search HTTP error for srcdb={srcdb}, keyword={keyword!r}. "
        ...
        if "json" not in content_type:
            preview = resp.text[:800]
            raise RuntimeError(
                f"Class search returned non-JSON for srcdb={srcdb}, keyword={keyword!r}. "
    ...
    # ---- Exhausted retries ----
    raise RuntimeError(
        f"Class search API never returned valid JSON after {max_tries} attempts "
        f"for srcdb={srcdb}, keyword={keyword!r}."
    )
...
def fetch_section_details(
    ...
    for attempt in range(1, max_tries + 1):
        ...
        if "json" not in content_type:
            preview = resp.text[:800]
            raise RuntimeError(
                f"Details returned non-JSON for code={code}, crn={crn}, srcdb={srcdb}. "
                f"Status={resp.status_code}, Content-Type={content_type!r}. "
                f"Body preview: {preview}"
            )
    ...
    # ---- Exhausted retries ----
    raise RuntimeError(
        f"Details API never returned valid JSON after {max_tries} attempts "
        f"for code={code}, crn={crn}, srcdb={srcdb}."
    )
